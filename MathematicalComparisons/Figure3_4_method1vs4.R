...
# Figure 3. Comparison of methods 1 and 4 power (bonf and sidak)
...
figure3 <- ggplot(plot14data1, aes(x = lambda4minus1, y = PowerDiff,
  ...
  labs(color = TeX("$\\pi^{PADJ} < \\pi^{DIS2DF}$"))
...
figure4 <- ggplot(plot14data2, aes(x = lambda4minus1, y = PowerDiff,
                                          group = Method4Bigger, color = Method4Bigger)) +
  geom_point() + theme(text = element_text(size = 15)) +
...
ggsave(filename = "./MathematicalComparisons/Output/Figure4.png",
       plot = figure4,
