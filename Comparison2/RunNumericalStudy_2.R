...
powerLong <- powerTable %>%
  ...
  mutate("Method Label" = fct_recode(Method,
                                     ...
                                     "4. Disjunctive 2-DF Test" = "method4_Chi2",
...
ggsave(filename = "./Comparison2/Results2/PowerDensity_2.png",
       plot = powerDensity,
       width  = 5000, height = 3000, units  = "px")
...
rank_summary_melted <- melt(rank_summary_table, id.vars = "Scenario") %>%
  ...
  mutate(Method = recode(Method,
                         ...
                         "method4_Chi2" = "Disjunctive 2-DF Test",
...
power15_names <- c("method1_bonf", "method5_MVN")
...
power15_diffs <- powerTable %>%
  ...
  dplyr::select(Scenario, K, m, eff2minus1_group, RhoCase, rho1, rho2, method1_bonf, method5_MVN) %>%
  mutate(method5minus1 = method5_MVN - method1_bonf)
...
power15_diffs_table <- power15_diffs %>%
  ...
  dplyr::select(-Scenario, -method1_bonf, -method5_MVN) %>%
...
write.csv(power15_table_summary,
          file = "./Comparison2/Results2/BONFvsCONJ/BONFvsCONJ_cases_2.csv")
write.csv(power15_diffs_table,
          file = "./Comparison2/Results2/BONFvsCONJ/BONFvsCONJ_diffs_2.csv")

# Analyzing CONJ vs. W1DF ------------------------------------------------------

# Vector of methods we want for this method (bonf and conj)
power35_names <- c("method3", "method5_MVN")
...
power35_diffs <- powerTable %>%
  ...
  dplyr::select(Scenario, K, m, eff2minus1_group, RhoCase, rho1, rho2, method3, method5_MVN) %>%
  mutate(method3minus5 = method3 - method5_MVN)
...
power35_diffs_table_summary <- power35_diffs_table %>%
  ...
  dplyr::select(-Scenario, -method3, -method5_MVN, -method3minus5) %>%
  dplyr::summarize(n = n()) %>%
...
write.csv(power35_table_summary,
          file = "./Comparison2/Results2/W1DFvsCONJ/W1DFvsCONJ_cases_2.csv")
write.csv(power35_diffs_table_summary,
          file = "./Comparison2/Results2/W1DFvsCONJ/W1DFvsCONJ_diffs_2.csv")
